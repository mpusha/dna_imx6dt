...
{
  for (int i=0;i<strlen((const char*)cmd);i++) cmd[i] = toupper(cmd[i]);
  // comands with reply
  if (!strncmp((const char *)cmd,"FVER",4))
    sprintf(reply,"DY>TFT %s",APP_VERSION);
...
{qDebug()<<"BulkClear"; return true;
  if(usb_handle){
    ...
    transfer.flags=LIBUSB_TRANSFER_FREE_TRANSFER|LIBUSB_TRANSFER_ADD_ZERO_PACKET ;
    transfer.endpoint=0x02;
    transfer.timeout=usb_tout;
    libusb_cancel_transfer(&transfer);
    qDebug()<<"BulkClear compl";
    return true;
  }
  return false;
}
...
void TUsb::timerEvent(QTimerEvent *ev)
{
  ev=ev;//remove warninig
  libusb_handle_events_completed(NULL, NULL); //process leave/arrive device
}
