...
//--- Constructor
//-----------------------------------------------------------------------------
TDtBehav::TDtBehav(int nPort, QString dName) : m_nNextBlockSize(0)
{
  ...
  logSys=new TLogObject(QApplication::applicationDirPath()+"/system");
  ...
  if(!m_ptcpServer->listen(QHostAddress::Any, nPort))
  {
    ...
    syslog( LOG_INFO, "Can't setup TCP connection. Network error!");
    qDebug()<<"Can't setup TCP connection. Network error!";
    logSys->logMessage("Can't setup TCP connection. Network error!");
    this->~TDtBehav();
    exit(0);
  }
  iniFileRead((QString&)(QApplication::applicationDirPath()+"/setup.ini"));
...
TDtBehav::~TDtBehav()
{
  ...
  logSys->logMessage("SDevice TDtBehav deinitialisation begin.");
  tAlrm.stop();
  BufVideo.clear();
  m_ptcpServer->close();
  ...
  logSys->logMessage("Device deinitialisation compleat.");
  ...
  parameters.clear();
  syslog( LOG_INFO, "Device deinitialisation compleat.");
  qDebug()<<"Device deinitialisation compleat.";
}
...
void TDtBehav::iniFileRead(QString& setingsFileName)
{
  ...
  logSize=setings.value("logFileSize",100000).toInt();
  logCount=setings.value("logFileCount",10).toInt();
