...
TDtBehav::~TDtBehav()
{
 ...
 // if(pClient) { delete pClient; pClient=0; }
...
void TDtBehav::iniFileRead(QString& setingsFileName)
{
  ...
  QString str=setings.value("vendorID","0x199a").toString();
  vendorID=str.toUInt(&ok,16); if(!ok)vendorID=0x199a;
  str=setings.value("productID","0x1964").toString();
  productID=str.toUInt(&ok,16); if(!ok)productID=0x1964;
...
// 2. INFODEV_serName
  ...
  if(!text.size()) if(USBCy_RW("FSN",answer,Optics_uC))
  map_InfoDevice.insert(INFODEV_serName, answer); // get device name from CAN
