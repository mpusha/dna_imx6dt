...
void TDtBehav::measHighTubeFinish(void)
{
  ...
  QString answer;
  int i=-1;
  if(USBCy_RW("HDIST",answer,Motor_uC)){
    QTextStream(&answer) >> i;
  }
  tubeHigh=i;
}
...
void TDtBehav::readAMPProg(void)
{
  ...
  while(1) {
    ...
    if(answer.contains("-1", Qt::CaseInsensitive)||answer.contains("?", Qt::CaseInsensitive)) { retVal=1; break; }
    QTextStream(&answer) >>type>>vol>>num_block>>num_level>>type_grad;
    ampProgList.append(QString("XPRG %1 %2 %3").arg(type).arg(vol).arg(type_grad));   // 1-ая строка программы
    for(i=0; i<num_block; i++) {    // цикл по кол-ву блоков
      if(!USBCy_RW(QString("XGB %1").arg(i),answer,Temp_uC)) { retVal=1; break; }
      if(answer.contains("-1", Qt::CaseInsensitive)) { retVal=1; break; }
      ...
      for(j=0; j<num_level_inBlock; j++){
        ...
        if(temp <=0 || time < 0) {retVal = 1; break; }
        ampProgList.append(QString("XLEV %1 %2 %3 %4 %5 %6").arg(temp).arg(time).
      ...
      if(retVal) break;
      switch (type_block) {
        case 1: text = QString("XCYC %1").arg(rep); break;
        case 3: text = "XHLD"; break;
        case 2: text = "XPAU"; break;
      }
      ampProgList.append(text);
    }
    if(USBCy_RW("XGN",answer,Temp_uC)) ampProgList.append(QString("XSAV %1").arg(answer));
    if(USBCy_RW("PONM",answer,Optics_uC)) ampProgList.append(QString("PONM %1").arg(answer)); // operator name
    if(USBCy_RW("PNUM",answer,Optics_uC)) ampProgList.append(QString("PNUM %1").arg(answer)); //protocol number
    if(USBCy_RW("PDATE",answer,Optics_uC)) ampProgList.append(QString("PDATE %1").arg(answer)); // date & time of run
    break;
  }
  if(retVal==1)devError.setDevError(GETAMPPROG_ERROR);
}
